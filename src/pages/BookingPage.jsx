...
import {
  ...
  query, where, onSnapshot, getCountFromServer
...
const BookingPage = () => {
  ...
  // ---- Watch date → load confirmed bookings for that day and disable times that overlap ----
  useEffect(() => {
  ...
  // Compute which time labels should be disabled using capacity
  const disabledTimes = useMemo(() => {
    if (!form.date) return new Set();

    // If day already hit the DAILY_CAPACITY, block all options
    if (confirmedForDay.length >= DAILY_CAPACITY) {
      return new Set(TIME_OPTIONS);
    }
  ...
  // ---- Submit → write requested booking with endAt/dateKey ----
  const handleProceedToCheckout = async () => {
    ...
    // Hard guard: if day already full, stop
    if (confirmedForDay.length >= DAILY_CAPACITY) {
      toast({
        variant: 'destructive',
    ...
    // Prevent selecting a disabled time
                ...
                <CardContent className="space-y-6">
                  {form.service !== 'office-cleaning' ? (
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <Select value={String(form.bedrooms)} onValueChange={(v) => handleFormChange('bedrooms', Number(v))}>
                            ...
                            <SelectContent>
                              {[...Array(9).keys()].map(i => <SelectItem key={i} value={String(i)}>{i}</SelectItem>)}
                            </SelectContent>
                          </Select>
                        </div>
                        <div>
                          ...
                          <Select value={String(form.bathrooms)} onValueChange={(v) => handleFormChange('bathrooms', Number(v))}>
                            <SelectTrigger><SelectValue /></SelectTrigger>
                            <SelectContent>
                              {[...Array(7).keys()].map(i => <SelectItem key={i} value={String(i)}>{i}</SelectItem>)}
                            </SelectContent>
                          </Select>
                        </div>
                      </div>
                    ...
                    <div>
                      ...
                      <Input id="sqft" type="number" value={form.sqft} onChange={(e) => handleFormChange('sqft', Number(e.target.value))} />
                    </div>
                  )}
                        ...
                        return (
                          <div key={time}>
                            ...
                            <Label
                              htmlFor={time}
                              className={[
                                'block p-3 border-2 rounded-lg text-center cursor-pointer transition',
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input id="name" value={form.name} onChange={e => handleFormChange('name', e.target.value)} required />
                    </div>
                    <div>
                      <Label htmlFor="email">Email</Label>
                      <Input id="email" type="email" value={form.email} onChange={e => handleFormChange('email', e.target.value)} required />
                    </div>
                  </div>
                  <div>
                    <Label htmlFor="address">Full Address</Label>
                    <Input id="address" value={form.address} onChange={e => handleFormChange('address', e.target.value)} required />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="phone">Phone</Label>
                      <Input id="phone" type="tel" value={form.phone} onChange={e => handleFormChange('phone', e.target.value)} required />
                    </div>
                    <div>
                      <Label htmlFor="zip">ZIP Code</Label>
                      <Input id="zip" value={form.zip} onChange={e => handleFormChange('zip', e.target.value)} required />
                    </div>
                  </div>
                  <div>
                      ...
                      <SelectContent>
                        {frequencies.map(freq => (
                          <SelectItem key={freq.id} value={freq.id}>
                            {freq.name} {freq.discount > 0 && `(${freq.discount * 100}% off)`}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
              ...
              <Card className="bg-light-pink/50">
                ...
                <CardContent className="space-y-2">
                  ...
                  <div className="border-t border-gold/30 my-2 pt-2">
                    ...
                    <div className="flex justify-between font-bold text-plum mt-2">
                    ...
                    <div className="flex justify-between text-2xl font-bold text-plum mt-2">
                      <span>Total</span>
                      <span>${estimate.total.toFixed(2)}</span>
                    </div>
                  </div>
                  <div className="text-xs text-plum/70 text-center pt-2 flex items-start gap-1">
