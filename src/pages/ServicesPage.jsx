    ...
    <main className="py-10 md:py-14 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <motion.div className="text-center mb-10 md:mb-16" {...fadeUp()}>
          <h2 className="text-3xl md:text-5xl font-bold text-plum mb-4">Our Cleaning Services</h2>
                ...
                <Card className="bg-white/90 border-gold/20 w-full flex flex-col transition-all duration-300 hover:shadow-lg hover:-translate-y-1 relative overflow-hidden">
                  ...
                  <CardFooter className="flex flex-col gap-1">
                    ...
                    <Link
                      ...
                      className="mt-1 inline-flex items-center justify-center gap-1 text-sm text-plum/70 hover:text-plum transition-colors"
                    >
          ...
          <div className="inline-flex flex-col md:flex-row items-center gap-4 md:gap-8 rounded-full bg-white p-4 shadow-sm">
            {perks.map((perk, index) => (
              <div key={index} className="flex items-center gap-2">
                <perk.icon className="w-5 h-5 text-gold" aria-hidden="true" />
        ...
        {/* 2) Popular Add-ons */}
        <div className="mt-16">
          <motion.h3 className="text-2xl font-bold text-plum mb-4" {...fadeUp()}>Popular Add-ons</motion.h3>
          <motion.div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4" {...fadeUp(0.05)}>
            {ADD_ONS.map((a) => (
              <div
                key={a.id}
                className="group flex items-center justify-between rounded-xl border border-gold/20 bg-white p-4 transition-all hover:shadow-sm hover:-translate-y-0.5"
              >
        ...
        <div className="mt-16">
          <motion.h3 className="text-2xl font-bold text-plum mb-4" {...fadeUp()}>Not sure which to pick?</motion.h3>
          <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" {...fadeUp(0.05)}>
            {SERVICES.map((s) => {
        ...
        {/* 4) How it works */}
        <div className="mt-16">
          <motion.h3 className="text-2xl font-bold text-plum mb-6" {...fadeUp()}>How it works</motion.h3>
          <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-6" {...fadeUp(0.05)}>
            {steps.map((st, i) => (
              <div key={st.title} className="rounded-2xl border border-gold/20 bg-white p-6 transition-all hover:-translate-y-0.5">
                <div className="w-12 h-12 rounded-full bg-gold/10 flex items-center justify-center mb-4">
                  <st.icon className="w-6 h-6 text-gold" aria-hidden="true" />
                </div>
          ...
          <motion.div {...fadeUp(0.05)}>
            <Card className="h-full flex flex-col border-gold/25 bg-white">
              <CardHeader className="space-y-2">
                <CardTitle className="text-plum text-2xl flex items-center gap-2">
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  {serviceAreas.map((tag) => (
                    <span key={tag} className="px-3 py-1 rounded-full bg-rose-50 border border-gold/20 text-plum text-xs font-medium">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              <CardContent className="flex-1">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {hours.map((h) => (
                    <div key={h.label} className="rounded-xl border border-gold/20 bg-white p-4 flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        <Clock4 className="w-5 h-5 text-gold" aria-hidden="true" />
