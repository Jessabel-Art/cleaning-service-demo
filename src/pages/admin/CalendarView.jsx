...
const CAL_HEIGHT = 520; // trimmed so the grid stays balanced
...
export function CalendarView() {
  ...
  // if auth known but not admin, show one precise message
  React.useEffect(() => {
    if (!authReady) return;
    if (!isAdmin) {
  ...
  return (
    <section>
      ...
      <style>{`
        /* keep rbc popups above shadcn dialogs if both exist */
      ...
      {selectedEvent && (
        <Dialog open={!!selectedEvent} onOpenChange={() => setSelectedEvent(null)}>
          {/* Force opaque overlay so the dialog is easy to read */}
          <style>{`
            .fixed.inset-0[data-state="open"] ~ * .dialog-overlay-force { background: rgba(0,0,0,0.6); }
          `}</style>
          <div className="dialog-overlay-force" />
