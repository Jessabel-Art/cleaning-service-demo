...
export function CalendarView() {
  ...
  // NEW: gate the listener on admin status
  ...
  React.useEffect(() => {
    const off = auth.onAuthStateChanged(async (u) => {
      ...
      // Log project + who we are (helps spot wrong-project config fast)
      try {
        const app = getApp();
        console.log("FB projectId:", app.options.projectId, "uid:", u.uid, "email:", u.email);
      } catch {}

      // Admin checks mirroring rules: /admins/{uid} OR email allowlist
      ...
      try {
        ...
        console.warn("admins/{uid} check failed", e);
      }
  ...
  // Subscribe only when admin is confirmed
  React.useEffect(() => {
  ...
  // If we know auth is ready but not admin, surface a precise message (don’t open the listener)
  React.useEffect(() => {
    if (!authReady) return;
    if (!isAdmin) {
      ...
      toast({
        ...
        description:
          `Signed in as ${email}. This account isn’t in /admins or the allowlist for this Firebase project. ` +
          `Add /admins/{uid} in the same project the app uses, or sign in with the owner account.`,
        variant: "destructive",
      });
    }
  ...
  const events = React.useMemo(() => {
    return rows
      .map((r) => {
        ...
        if (!end && start) {
          const minutes = Number(r.durationMinutes ?? (r.durationHours ? r.durationHours * 60 : 120));
  ...
  const eventStyleGetter = (event) => {
    ...
    return { style: { backgroundColor: bg, borderRadius: 8, padding: "2px 6px", color: "#111", border: "1px solid rgba(0,0,0,0.05)" } };
  };

  const hasConflict = (candidate, allEvents) => {
    const sameDay = allEvents.filter((e) => e.start.toDateString() === candidate.start.toDateString());
  ...
  const persistWhen = async (event, start, end) => {
    try {
      ...
      toast({
        title: "Rescheduled",
        description: `${start.toLocaleString()} – ${end.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`,
      });
    } catch (e) {
  ...
  const sidebarEvents = React.useMemo(() => {
    ...
    return filteredEvents.filter((e) => overlap(e.start, e.end, start, end)).sort((a, b) => a.start - b.start);
    ...
    <section>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <input type="date" value={customStart} onChange={(e) => setCustomStart(e.target.value)} className="px-3 py-2 rounded-lg border bg-white text-sm" aria-label="Start date" />
          <span className="text-plum/70">to</span>
          <input type="date" value={customEnd} onChange={(e) => setCustomEnd(e.target.value)} className="px-3 py-2 rounded-lg border bg-white text-sm" aria-label="End date" />
          <Button variant="ghost" size="sm" onClick={() => { setCustomStart(""); setCustomEnd(""); }}>Clear</Button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[2fr,1fr] gap-4">
        ...
        <aside className="bg-white border rounded-lg p-3">
          ...
          {!selectedRange && <div className="text-sm text-plum/70 mb-2">Select a date or drag over a range on the calendar to see appointments here.</div>}
          {selectedRange && sidebarEvents.length === 0 && <div className="text-sm text-plum/70">No appointments in the selected range.</div>}
          ...
          <DialogContent>
            ...
            <DialogFooter>
              <div className="flex gap-2">
                ...
                <Button variant="outline" onClick={() => printEvent(selectedEvent)}>Print</Button>
                <Button className="bg-plum text-white" onClick={() => { navigate(`/book?bookingId=${selectedEvent.id}`); setSelectedEvent(null); }}>
                  Reschedule
                </Button>
              </div>
            </DialogFooter>
